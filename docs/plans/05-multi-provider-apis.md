...
> Status: **partially landed** — `ModelBackend` + Gemini/Groq/OpenRouter backends + failover + cost classes + semaphore + explain cache + usage/audit telemetry are in (`backend/ai/{service,backend,openai_compat}.go`, `backend/usage/{usage,policy}.go`, `backend/tools/`); Settings shows live meters via `App.AIMeters`. Still open: tertiary OpenRouter default, map-reduce multi-source, soft daily budgets, persisted UsageEvents.
